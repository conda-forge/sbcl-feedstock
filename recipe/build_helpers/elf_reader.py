...
def get_uniqueness_key(file):
    ...
    if not binary:
        ...
        dynamic_entries = binary.dynamic_entries
        print(f"**** DBG ****: Dyn. Ent.:\t{dynamic_entries}")
        for entry in dynamic_entries:
            print(f"**** DBG ****: \t{entry}:{entry.tag}:{entry.name if hasattr(entry, 'name') else '-'}")

        result = [
            e.name for e in dynamic_entries if e.tag == lief.ELF.DYNAMIC_TAGS.SONAME
        ]
        if result:
            print(f"**** DBG ****:   '-> Result:\t{result[0]}")
            return result[0]
        print(f"**** DBG ****: \t{dir(binary)}")
        snapshot = tracemalloc.take_snapshot()
        display_top(snapshot)

        try:
            print(f"**** DBG ****: \t{(binary.__str__())}")
        except Exception as e:
            snapshot = tracemalloc.take_snapshot()
            display_top(snapshot)
            print(f"**** DBG ****: \t{e}")
        return str(file)
    print(f"**** DBG ****: \t{len(file)}")
    return str(file)
...
def main():
    ...
    print(f"\n\n**** DBG ****: {get_linkages(args.elf_file)}\n\n")
